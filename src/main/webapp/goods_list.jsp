						...
						<ul class="ranklist" id='ranklist'>
							<c:forEach items="${goodsesWithOrder }" var="goods" varStatus="s">
								<li><span>${s.index+1 }</span> <a title="${goods.name }"
									class="p_name" href="goods_view?goods.id=${goods.id }">${goods.name }
			...
			<div class="main f_r">
				...
				<div class="display_title">
					...
					<ul class="tag">
						...
						<li id="sellnum" class="current"><span class="l"></span><span
							class="r"></span> <a
							href="goods_listByCate?goods.categoryId=${category.id}&order=sellnum&index=1">销量<span
								class="">&nbsp;</span></a> <input id="index" name="index"
							type="hidden" value="${index }" /></li>
						<li id="role" class="" onclick=""><span class="l"></span><span
							class="r"></span> <a
							href="goods_listByCate?goods.categoryId=${category.id}&order=role&index=2">热度<span
								class="">&nbsp;</span></a></li>
						<li id="price2" class=""><span class="l"></span><span
							class="r"></span> <a
							href="goods_listByCate?goods.categoryId=${category.id}&order=price2&index=3">价格<span
								class="">&nbsp;</span></a></li>
						<li id="selltime" class=""><span class="l"></span><span
							class="r"></span> <a
							href="goods_listByCate?goods.categoryId=${category.id}&order=selltime&index=4">最新上架<span
								class="">&nbsp;</span></a></li>
					</ul>
				</div>
				...
				<ul class="display_list clearfix m_10">
					...
					<c:forEach items="${goodses }" var="goods">
						<li class="clearfix win"><input type="hidden" name="page"
							id="page" value="" />
							<div class="pic">
								<a title="${goods.name }"
									href="goods_view?goods.id=${goods.id }"><img
									src="${goods.thumbnail }" width="200" height="200"
									alt="${goods.name }" title="${goods.name }" /></a>
							</div>
							<h3 class="title">
								<a title="${goods.name }" class="p_name" href="goods_view?goods.id=${goods.id }">${goods.name }</a><span>总销量：${goods.sellNum }<a
							...
							<div class="handle">
								<label class="btn_gray_m"><img
									...
									<input type="button" value="加入购物车" onclick="joinCart_list(1);"></label>
